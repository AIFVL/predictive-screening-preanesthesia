...
# SECCIÓN 0: TÍTULO
# ============================================================================
cells.append(md_cell("""# 7. Modelado Predictivo (Modular) — Screening Preanestésico

**Objetivo**: Predecir si un paciente requiere valoración preanestésica (`target=1`).
**Estrategia Fase 2**: Maximizar ROC-AUC (Ranking) para luego ajustar el umbral operativo.
**Población**: Solo pacientes **adultos** (Edad >= 18 años).

> [!NOTE]
> Este notebook utiliza Smart Imputation, HistGradientBoosting y optimización por ROC-AUC."""))

# ============================================================================
# SECCIÓN 1: SETUP
# ============================================================================
cells.append(md_cell("## 1. Setup y Carga de Módulos"))
...
if current_dir.endswith('notebook') or current_dir.endswith('notebooks'):
...
from utils.feature_engineering import resolve_feature_columns, get_imputation_strategies, sanitize_features_for_subset, ENCODING_FIX_MAP
...
# ============================================================================
cells.append(md_cell("## 2. Carga y Preparación de Datos"))

cells.append(code_cell("""# 2.1 Cargar Dataset Maestro
df = load_opera_completo()
print(f"Dataset completo: {df.shape[0]:,} registros")

# 2.2 FILTRO: Solo pacientes adultos (Edad >= 18)
n_antes = len(df)
df = df[df['Edad'] >= 18].reset_index(drop=True)
n_despues = len(df)
print(f"Filtro adultos (Edad >= 18): {n_antes:,} -> {n_despues:,} registros")
print(f"  Pacientes pediátricos excluidos: {n_antes - n_despues:,}")"""))

cells.append(code_cell("""# 2.3 Cargar Metadatos de Features
features_meta = load_features_metadata()
selected_names = features_meta['Variable'].tolist()

# 2.4 Resolver Columnas de Features
...
if missing:
    print(f"Features NO encontradas ({len(missing)}): {missing}")
else:
    print("Todas las features fueron resueltas correctamente.")"""))

cells.append(code_cell("""# 2.5 Corregir Encoding en Nombres de Columnas
df = df.rename(columns=ENCODING_FIX_MAP)
feature_columns = [ENCODING_FIX_MAP.get(c, c) for c in feature_columns]

# 2.6 Sanitizar Features (Eliminar Constantes en Subset Adultos)
print("\\n--- Sanitización de Features (Adultos) ---")
...
print(f"Prevalencia target=1: {y.mean():.4f} ({y.sum():,} de {len(y):,})")"""))

# ============================================================================
# SECCIÓN 3: PREPROCESAMIENTO AVANZADO
# ============================================================================
cells.append(md_cell("## 3. Split y Smart Imputation"))

cells.append(code_cell("""from sklearn.model_selection import StratifiedShuffleSplit
...
# 3.2 Definir Estrategias de Imputación (Smart Imputation)
...
print(f"  - Fill Zero (Scores/Counts): {len(cols_zero)} variables")
print(f"  - Fill Median (Medidas):     {len(cols_median)} variables")

# 3.3 Construir ColumnTransformer
...
try:
    ...
    print("Smart Imputation completada.")
except Exception as e:
    print(f"Error en imputación: {e}")
    print("Aplicando fallback a imputación simple...")
    imputer = SimpleImputer(strategy='median')
    X_train_tree = pd.DataFrame(imputer.fit_transform(X_train), columns=X_train.columns)
    X_test_tree = pd.DataFrame(imputer.transform(X_test), columns=X_test.columns)
    X_train_linear = X_train_tree
    X_test_linear = X_test_tree
"""))

# ============================================================================
# SECCIÓN 4: MODELADO
# ============================================================================
cells.append(md_cell("## 4. Entrenamiento y Evaluación Comparativa"))

cells.append(code_cell("""# 4.1 Definir Modelos Base
...
print(f"\\n{'='*60}")
print("RANKING DE MODELOS (por ROC-AUC) - Solo Adultos")
print(f"{'='*60}")
print(df_results)"""))
...
# ============================================================================
cells.append(md_cell("## 5. Visualización de Resultados"))

cells.append(code_cell("""# 5.1 Matrices de Confusión
viz.plot_confusion_matrices(predictions, y_test, save_path='../matrices_confusion_phase2.png')"""))

cells.append(code_cell("""# 5.2 Curvas ROC y PR
viz.plot_roc_pr_curves(predictions, y_test, save_path='../curvas_roc_pr_phase2.png')"""))

# ============================================================================
# SECCIÓN 6: OPTIMIZACIÓN
# ============================================================================
cells.append(md_cell("## 6. Optimización (Optuna) del Mejor Modelo"))

cells.append(code_cell("""# 6.1 Identificar el Mejor Modelo (por ROC-AUC)
best_model_name = df_results.index[0]
best_auc_initial = df_results.iloc[0]['ROC-AUC']

print(f"Mejor modelo discriminativo: {best_model_name} (AUC={best_auc_initial:.4f})")
print("Iniciando optimización de hiperparámetros (Target: Maximizar ROC-AUC)...")

# Seleccionar datos
if best_model_name == 'Regresión Logística':
    X_train_opt = X_train_linear
    X_test_opt = X_test_linear
else:
    X_train_opt = X_train_tree
    X_test_opt = X_test_tree

# 6.2 Optimizar (Target ROC-AUC)
...
cells.append(code_cell("""# 6.3 Re-entrenar con mejores parámetros
...
cells.append(code_cell("""# 6.4 Optimización de Umbral (Post-hoc)
...
print(f"Umbral Optimo para F2: {opt_thresh:.2f}")
print(f"F2-Score Final: {opt_f2:.4f}")"""))

# ============================================================================
# SECCIÓN 7: INTERPRETABILIDAD
# ============================================================================
cells.append(md_cell("## 7. Interpretabilidad (SHAP)"))

cells.append(code_cell("""import shap
import numpy as np

print(f"Explicando modelo: {type(final_model).__name__}")

try:
    if best_model_name in ['XGBoost', 'Random Forest', 'Árbol de Decisión', 'HistGradientBoosting']:
    ...
    print(f"Error inicializando SHAP ({e}). Usando fallback KernelExplainer...")
    background = shap.kmeans(X_train_opt, 10)
    explainer = shap.KernelExplainer(final_model.predict_proba, background)
    shap_explanation = explainer.shap_values(X_test_opt.iloc[:50])
    if isinstance(shap_explanation, list): shap_explanation = shap_explanation[1]

# Procesar shap_explanation
if isinstance(shap_explanation, np.ndarray):
    vals = shap_explanation
...
cells.append(code_cell("""# 7.2 Top Features Dependence
if len(vals) > 0:
...
cells.append(code_cell("""# 7.3 Ejemplo Local (Waterfall)
...
if len(tp_indices) > 0:
    ...
    viz.plot_shap_waterfall(vals, explainer, X_test_opt, idx, save_path='../shap_waterfall_phase2.png', expected_value=ev)"""))
...
with open("notebook/7_modeling.ipynb", "w", encoding="utf-8") as f:
    json.dump(notebook_json, f, indent=1, ensure_ascii=False)

print("Notebook '7_modeling.ipynb' (Phase 2 - Solo Adultos) regenerado exitosamente.")
